...
use syn::{
    Data, DataEnum, DataStruct, DeriveInput, Error, Expr, ExprLit, ExprPath, Field, Fields, Ident,
...
pub fn expand(input: &DeriveInput) -> Result<TokenStream> {
    ...
    match &input.data {
        Data::Enum(data_enum) => {
            ...
            expand_enum(ident, data_enum, &state)
        }
        Data::Struct(data_struct) => expand_struct(ident, data_struct),
...
fn expand_enum(ident: &Ident, data_enum: &DataEnum, state: &ExprPath) -> Result<TokenStream> {
...
fn expand_struct(ident: &Ident, data_struct: &DataStruct) -> Result<TokenStream> {
...
fn expand_field(field: &Field) -> Result<TokenStream> {
        ...
        .is_some_and(char::is_lowercase)
    {
        ...
        Ok(quote_spanned! { field.span() =>
            #field_ident: <super::#field_ty as Deserialize>::deserialize(d)?,
        })
    }
}
